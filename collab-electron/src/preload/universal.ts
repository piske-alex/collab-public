...
contextBridge.exposeInMainWorld("api", {
  ...
  // Terminal (PTY)
  ptyCreate: (cwd?: string, cols?: number, rows?: number, shell?: string) =>
    ipcRenderer.invoke(
      "pty:create",
      { cwd, cols, rows, shell },
    ),
  ...
  notifyPtySessionId: (sessionId: string, shell?: string) =>
    ipcRenderer.sendToHost("pty-session-id", sessionId, shell),
